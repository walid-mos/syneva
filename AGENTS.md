...
**Agent contract:** plain JSON on stdout. `syneva await` long-polls and prints one tagged event — `{"kind":"question",…}` (answer now via `syneva comment`) or `{"kind":"review","result":{…ReviewResult…}}` (the reviewer hit Send). The contract is the single source of truth in `src/spec.ts` (printed by `syneva spec`); the skill (`skills/syneva/SKILL.md`) is bootstrap-only and points consuming agents at `syneva spec`, and the server's error responses do too. **If you change the CLI flags, events, or ReviewResult shape, update `src/spec.ts` in the same change.** `src/spec.test.ts`, `src/cli.test.ts`, and `src/server.test.ts` cover the documented contract and its CLI/HTTP behavior.
