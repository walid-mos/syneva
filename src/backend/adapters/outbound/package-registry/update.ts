...
import { existsSync, promises as fs, realpathSync, readFileSync } from 'node:fs'
...
export function currentVersion(): string {
	try {
		const pkg: unknown = JSON.parse(readFileSync(packageJsonUrl(), 'utf8'))
...
function packageJsonUrl(): URL {
	...
	for (;;) {
		...
		if (parent.pathname === dir.pathname) return candidate
		dir = parent
	}
}
