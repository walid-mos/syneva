...
void test('serialize runs tasks in enqueue order - a later task waits for an earlier slow one', async () => {
	...
	assert.ok(isDeepStrictEqual(order, ['slow', 'fast']))
	assert.deepEqual(results, ['slow', 'fast'])
})

void test('a rejecting task does not poison the chain, and its rejection reaches the caller', async () => {
	const serialize = createSerializer()
	const order: string[] = []
	...
	assert.ok(isDeepStrictEqual(order, ['boom', 'after']))
})
