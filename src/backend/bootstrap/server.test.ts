...
import { writeGlobalSettings } from '../adapters/outbound/filesystem/desk.js'
...
import type { BrowserReviewState } from '../../contracts/browser.js'
...
void test('Send flushes queued questions and folds the unanswered ones into openQuestions', async () => {
	await withServer(
		async (handle, _root, st) => {
			...
			assert.ok(isDeepStrictEqual(st.comments, QUESTION_COMMENTS))
			await post(handle.url, 'api/ask', {
				path: 'a.ts',
				lineNumber: 1,
				body: 'why q1?',
		...
		{ seed: { comments: QUESTION_COMMENTS } },
	)
})
...
void test('send survives bodies beyond the old 5 MB cap and merges only the reviewer slice', async () => {
	await withServer(async handle => {
		...
		assert.ok(isDeepStrictEqual(handle.state().reviewedFiles, ['a.ts']))
		assert.equal(handle.state().rawDiff, '')
		...
		assert.equal('overallNote' in handle.state(), false)
	})
})
...
void test('save strips transient desk-status keys so they never persist on state', async () => {
	await withServer(async handle => {
		...
		assert.equal('agentActivity' in handle.state(), false)
		assert.equal('agentListening' in handle.state(), false)
		assert.equal('queuedQuestions' in handle.state(), false)
		assert.equal('queuedReviews' in handle.state(), false)
	})
})
...
void test('save merges the reviewer-owned slice and leaves server-owned state authoritative', async () => {
	await withServer(
		async handle => {
			...
			assert.equal(res.status, 200)
			assert.ok(isDeepStrictEqual(handle.state().reviewedFiles, ['a.ts']))
			assert.deepEqual(handle.state().decisionFiles, ['a.ts'])
			assert.equal(handle.state().decisions?.[0]?.status, 'accepted')
			// Server-owned fields are untouched - the wire never carried them.
			assert.equal(handle.state().rawDiff, 'SERVER DIFF')
			assert.equal(handle.state().files.length, 1)
			assert.equal(handle.state().files[0].contentHash, 'H')
		},
...
void test('save from a stale full-state tab ignores server-owned fields in the body', async () => {
	await withServer(async handle => {
		const before = handle.state().files[0].contentHash
		...
		assert.ok(isDeepStrictEqual(handle.state().reviewedFiles, ['a.ts']))
		assert.equal(handle.state().rawDiff, '')
		assert.equal(handle.state().baseDiffHash, 'base')
		assert.equal(handle.state().id, 'id')
		assert.equal(handle.state().files.length, 1)
		assert.ok(Object.is(handle.state().files[0].contentHash, before))
	})
})
...
void test('/api/stage: paths[] stages a move pair as a rename; legacy {path} still works (issue 02)', async () => {
	await withServer(async (handle, root) => {
		...
		assert.ok(handle.state().stagedFiles.includes('new.ts'))
		assert.ok(handle.state().stagedFiles.includes('extra.ts'))
		assert.ok(!handle.state().stagedFiles.includes('old.ts'))
	})
})
...
void test('/api/reload: a posted grouping replaces the carried guide, is stamped to the new diff, and an invalid one is 422 (desk untouched)', async () => {
	await withServer(async (handle, root) => {
		...
		const attached = handle.state().guide
		assert.ok(
			attached,
			'the reload should have attached the posted grouping',
		)
		assert.equal(attached.baseDiffHash, handle.state().baseDiffHash)
		assert.ok(
		...
		const regrouped = handle.state().guide
		...
		assert.ok(isDeepStrictEqual(handle.state().guide, before))
	})
})
...
void test('/api/file-contents returns contents equal to the embedded copies; rejects escapes + unknown paths (issue 02)', async () => {
	...
	await writeFile(path.join(root, 'a.ts'), 'one\nCHANGED\nthree\n')
	await writeFile(path.join(root, 'b.ts'), 'brand new\n')
	const st = await buildReviewState(root, { session: 's' }, nodeGit)
...
void test('a throwing wrapped route settles the mutex without poisoning the chain (issue 05)', async () => {
	await withServer(async handle => {
		...
		// promise) while the caller still sees the malformed-body 400, not the INTERNAL 500 fallback.
		...
		assert.equal(badBody.code, 'INVALID_JSON')
...
void test('/api/reset unstages every reviewed file in one batched restore and clears the review (issue 13)', async () => {
	...
	const built = await buildReviewState(root, { session: 's' }, nodeGit)
	assert.ok(built, 'built a review state for the working diff')
	g(['add', '.'])
	assert.deepEqual(
	...
	// Review records are published immutable, so the reviewer slice is seeded up front.
	...
	try {
		...
		assert.deepEqual(handle.state().comments, [])
		assert.deepEqual(handle.state().decisions, [])
	} finally {
		handle.server.close()
		process.env.HOME = oldHome
		await rm(root, { recursive: true, force: true })
