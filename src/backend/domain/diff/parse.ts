...
import type { DiffFile, DiffHunk, WritableDeep } from '../review.js'
...
class DiffParser {
	...
	private startFile(rawLine: string): void {
		const file: WritableDeep<DiffFile> = { hunks: [] }
...
export function parseUnifiedDiff(raw: string): readonly DiffFile[] {
	gitStats.parses++
