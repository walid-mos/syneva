...
void test('validateGuide accepts a well-formed guide and sorts files by order', () => {
	const r = validateGuide(validInput())
	assert.ok(r.ok) // narrows `r` to the accepted variant
...
void test('validateGuide defaults missing order (by position) and category', () => {
...
void test('validateGuide accepts skim fields and normalizes skimBlocks lines', () => {
	const r = validateGuide({
		overview: 'o',
		files: [
			{
	...
	assert.deepEqual(f.skimBlocks, [
		{ lines: [5, 5], reason: 'import' },
		{ lines: [10, 12] },
	])
})

void test('validateGuide leaves skim fields unset when absent', () => {
	const r = validateGuide({
		overview: 'o',
		files: [{ path: 'a.ts', orientation: 's' }],
	})
...
void test('validateGuide carries focused as a boolean and rejects other types (issue 04)', () => {
	...
	assert.ok(on.ok)
	assert.equal(on.guide.focused, true)
	// Omitted → undefined (a plain guide renders no badge).
	const off = validateGuide({
		overview: 'o',
		files: [{ path: 'a.ts', orientation: 's' }],
	})
	assert.ok(off.ok)
	assert.equal(off.guide.focused, undefined)
...
void test('validateGuide accepts movedFrom and rejects it with skimBlocks (issue 03)', () => {
	// movedFrom alone (and with whole-file skim) is fine - shape only; diff resolution is later.
	const ok = validateGuide({
		overview: 'o',
		files: [
			{ path: 'b.ts', orientation: 's', movedFrom: 'a.ts', skim: true },
		],
	})
	assert.ok(ok.ok)
	...
	assert.equal(
		validateGuide({
			overview: 'o',
			files: [{ path: 'b.ts', orientation: 's', movedFrom: '  ' }],
		}).ok,
		false,
	)
...
// A guide whose one file carries the given (malformed) skimBlocks value.
function bad(skimBlocks: unknown): boolean {
	return validateGuide({
		overview: 'o',
		files: [{ path: 'a.ts', orientation: 's', skimBlocks }],
	}).ok
}

void test('validateGuide rejects malformed skimBlocks', () => {
...
void test('validateGuide rejects malformed input', () => {
	...
	for (const input of cases)
		assert.equal(validateGuide(input).ok, false, JSON.stringify(input))
})
...
void test('mergeReviewState carries an attached guide across a reload', async () => {
	const guide: Guide = {
		overview: 'o',
		files: [
			{ path: 'a.ts', order: 0, category: 'Config', orientation: 's' },
		],
	}
