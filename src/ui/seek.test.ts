...
const finishedIn = (done: number[]): ((i: number) => boolean) => {
	const s = new Set(done)
	return (i: number) => s.has(i)
}

void test('nextUnreviewed wraps past the end to the first unreviewed file - guide order', () => {
...
void test('nextUnreviewed wraps past the end to the first unreviewed file - file order', () => {
	const order = [0, 1, 2, 3, 4]
	// Only file 2 remains; cur is the last file (4).
	assert.equal(nextUnreviewed(order, 4, finishedIn([0, 1, 3, 4])), 2)
})

void test('nextUnreviewed returns null when the current file is the last remaining one', () => {
...
void test('nextUnreviewed never returns the current file, even when it is unreviewed', () => {
...
void test('nextUnreviewed treats a hash-invalidated (dropped-out) file as a wrap target', () => {
...
void test('nextUnreviewed starts from the top when cur is not in the order', () => {
...
void test("partial guide: approve-advance and wrap reach the unlisted 'Other' files", () => {
...
void test('wrapNextTarget lands on the first unreviewed file when work remains', () => {
	assert.equal(
		wrapNextTarget([3, 1, 4, 0, 2], finishedIn([3, 1])),
		4,
		'guide order',
	)
...
void test('wrapNextTarget cycles to the first file when everything is reviewed', () => {
	assert.equal(wrapNextTarget([3, 1, 4], finishedIn([3, 1, 4])), 3)
})

void test('wrapPrevTarget lands on the last unreviewed file when work remains', () => {
	assert.equal(
		wrapPrevTarget([3, 1, 4, 0, 2], finishedIn([2, 0])),
		4,
		'guide order',
	)
...
void test('wrapPrevTarget cycles to the last file when everything is reviewed', () => {
