...
void test('a file-level skim flag makes the file fully skimmed regardless of blocks', () => {
	assert.equal(isFullySkimmed(true, []), true)
	assert.equal(isFullySkimmed(true, [false, false]), true)
})

void test('a file is fully skimmed when it has blocks and every one is skim-stamped', () => {
	assert.equal(isFullySkimmed(false, [true]), true)
	assert.equal(isFullySkimmed(false, [true, true, true]), true)
})

void test('a partially-skimmed file (any unskimmed block) stays a normal file', () => {
	assert.equal(isFullySkimmed(false, [true, false]), false)
	assert.equal(isFullySkimmed(false, [false, true, true]), false)
})

void test('a file with no change blocks and no file-level flag is not fully skimmed', () => {
...
void test('dropping the last skim stamp on reload returns the file to the flow', () => {
