...
const cacheKey = (f: ReviewFile): string => `${f.path}\0${f.contentHash}`
...
export function peekContents(f: ReviewFile): Contents | null {
	if (S.preview && f === S.preview)
		return {
			oldContents: S.preview.previewContents,
			newContents: S.preview.previewContents,
		}
...
async function prefetch(f: ReviewFile): Promise<void> {
	try {
...
export async function loadCurrentContents(): Promise<'ok' | 'stale' | 'error'> {
	...
	try {
		...
		if (currentFileOrNull() !== f) return 'stale' // switched files mid-fetch - drop this pass
		cur.path = f.path
		cur.oldContents = oldContents
		cur.newContents = newContents
		return 'ok'
	} catch {
		if (currentFileOrNull() !== f) return 'stale'
		return 'error'
	}
}
