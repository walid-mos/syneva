import { currentFileComments, currentFileOrNull } from './changes'
...
export function fileCommentIconButton(): HTMLElement {
	...
	b.className = 'fc-btn'
	if (S.fileComposerOpen) b.classList.add('active')
	b.setAttribute(
		'data-tip',
		S.fileComposerOpen ? 'Close file comment (⇧C)' : 'Comment on file (⇧C)',
	)
...
function standaloneComposer(): HTMLElement | null {
	if (!S.fileComposerOpen || S.editingCommentId) return null
...
function markdownBar(): HTMLElement {
	...
	label.textContent = `Comment on ${
		currentFileOrNull()?.path.split('/').pop() ?? 'this file'
	}`
