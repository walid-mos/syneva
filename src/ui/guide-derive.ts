// Pure guide-staleness check (store-free, so it's unit-testable - the same pattern as
// walkthrough.ts/linemap.ts). A guide is stale when it carries a baseDiffHash that no longer matches the diff now
