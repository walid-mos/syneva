...
import { S, persist, requireState, toast, esc } from './store'

import type { ReviewComment, ThreadMeta } from './types'
...
function messageHtml(message: ReviewComment, thread: ThreadMeta): string {
	...
	const isEditing = S.editingCommentId === message.id
	const isEdited =
		message.updatedAt &&
		message.createdAt &&
...
function setThreadStatus(
	...
	for (const comment of requireState().comments) {
		const isSameAnchor =
...
function wireThreadActions(box: HTMLElement, thread: ThreadMeta): void {
	...
	reply?.addEventListener('click', () => {
		...
		S.selected = {
			side: thread.side,
			lineNumber: toDisplayLine(thread.side, thread.lineNumber),
		}
		openComposer()
	})
...
export function buildCommentThread(thread: ThreadMeta): HTMLElement {
	...
	if (S.editingCommentId && thread.status !== 'resolved') {
		const message = box.querySelector(
			`.msg[data-id="${S.editingCommentId}"]`,
		)
