import { CURATED_LANGS } from '@shared/highlighting/shiki-langs'
import { CURATED_THEMES } from '@shared/highlighting/shiki-themes'
...
// @pierre/diffs pulls from `shiki`, backed by that same curated set (see shiki-langs.ts /
...
type CoreOptions = Parameters<typeof createHighlighterCore>[0]

export function createHighlighter(options: {
	engine: CoreOptions['engine']
...
export function createOnigurumaEngine(): ReturnType<
	typeof createJavaScriptRegexEngine
> {
...
function plainLoader(lang: string): LangLoader {
	const cached = plainLoaderCache.get(lang)
	if (cached) return cached
	const loader = (): Promise<{ default: unknown }> =>
		Promise.resolve({ default: plainGrammar(lang) })
...
export const bundledLanguages: Record<string, LangLoader> = new Proxy(
	curatedLanguages,
	{
		get(target, prop, receiver) {
			if (typeof prop === 'string' && !(prop in target))
				return plainLoader(prop)
...
for (const name of Object.keys(CURATED_THEMES))
	curatedThemes[name] = () =>
		Promise.resolve({ default: CURATED_THEMES[name] })
