...
void test('reviewerSlice carries only reviewer-owned fields - no rawDiff, no file contents', () => {
	// A full state INCLUDING backend-only fields (rawDiff, the aggregate ids/timestamps) the
	// browser projection never carries, so the assertions below actually prove the save slice
	// drops them. The Record side of the intersection stands in for the server-side state type:
	// the wire body historically carried a whole old state, and reviewerSlice must ignore every
	// field it doesn't pick.
	const full: ReviewState & Record<string, unknown> = {
		id: 'id',
		session: 's',
		root: '/r',
		repoHash: 'h',
	...
	// The serialized body carries none of the server-owned state.
