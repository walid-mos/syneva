...
import { cur, peekContents } from '@entities/review/file/contents'
...
function metadataFingerprint(file: ReviewFile, view: DiffView): string {
	...
	return [
		...
		diffCtx().S.settings.hideReviewed ? 'h' : 'v',
		changes,
	].join('\x00')
}
...
function buildDiffMetadata(file: ReviewFile, view: DiffView): FileDiffMetadata {
	...
	ensureChangesFromFileDiff(
		...
		diffCtx().S.state ??
			(() => {
				throw new Error('review state read before the first fetch')
			})(),
		file,
	)
...
export function prefetchableParse(
	...
	if (
		diffCtx().S.state?.decisions?.some(d =>
			d.key.startsWith(`${file.path}:`),
		)
	)
		return undefined
	const contents = peekContents(file, diffCtx().S.preview)
