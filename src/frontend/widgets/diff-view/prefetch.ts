...
import { isExpandCapped } from '@shared/diff-renderer/expand-cap'
...
export function warmNextFileTokens(file: ReviewFile, view: DiffView): void {
	...
	// "next".
	const files = diffCtx().S.state?.files
	...
	const stamp = [
		next.path,
		next.contentHash,
		diffCtx().S.settings.unchangedLines === 'expand' ? 'e' : 'c',
...
function cachedContents(
	...
	prefetchContents(file, diffCtx().S.loadedOversized)
	const cached = peekContents(file, diffCtx().S.preview)
