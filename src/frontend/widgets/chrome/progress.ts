...
const GI = (): GuideInputs => ({
	state: chromeCtx().S.state,
	fileIndex: chromeCtx().S.fileIndex,
	hideReviewed: chromeCtx().S.settings.hideReviewed,
	progressBy: chromeCtx().S.settings.progressBy,
	foldExpanded: chromeCtx().S.foldExpanded,
})
...
export function updateProgress(): void {
	...
	const hasFiles = !!chromeCtx().S.state?.files.length
...
export function reviewStats(): {
	...
	const { outOfFlow } = flowIndex(chromeCtx().S.state, {
		distill: chromeCtx().S.settings.hideReviewed,
	})
	const scope = (chromeCtx().S.state?.files ?? []).filter(
		f => !outOfFlow.has(f.path),
	)
	let lines = 0
	...
	return {
		files: scope.length,
		lines,
		comments: (chromeCtx().S.state?.comments ?? []).filter(
			c => c.role === 'user' && c.status === 'open',
		).length,
		rejections: (chromeCtx().S.state?.changes ?? []).filter(
			c => c.status === 'rejected',
		).length,
	}
}
