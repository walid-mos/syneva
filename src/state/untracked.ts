...
import { blobOid, fileAt } from '../git/repo.js'

import { fileEntry } from './diff-files.js'

import type { ChangeState, ReviewFile } from '../types.js'
...
export async function appendUntrackedFiles(
	...
	const entries = await readUntrackedEntries(scan)
	const pairing = await pairUntrackedMoves(scan.root, files, entries)
...
async function readUntrackedEntries(
	...
	const untracked = (await git(args, scan.root))
		.split(/\r?\n/)
		.filter(Boolean)
	...
	return await mapContentReads(untracked, async rel => ({
		rel,
		working: await fs
			.readFile(path.join(scan.root, rel), 'utf8')
			.catch(() => ''),
	}))
}
...
async function deletionsByOid(
	...
	const hashed = await mapContentReads(deletions, async deletion => ({
		deletion,
		oid: blobOid(
			await fileAt(root, deletion.oldPath ?? deletion.path, ':0'),
		),
	}))
