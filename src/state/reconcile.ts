...
import { effectiveDecisions } from './decisions.js'
...
export async function mergeReviewState(
	...
	return {
		...
		comments: await mergeComments(base, saved, rename),
...
function renameMigration(files: ReviewFile[]): RenameMigration {
...
function mergeDecisions(
	...
	for (const change of base.changes) {
		...
		if (
			...
			change.reviewedHash = decision.reviewedHash
			continue
		}
...
async function mergeComments(
	...
	const contentsByPath = await readCommentedContents(base, openPaths)
...
async function readCommentedContents(
	...
	await mapContentReads(commented, async file => {
		const resolved = await readFileContents(base, file).catch(
			() => undefined,
		)
