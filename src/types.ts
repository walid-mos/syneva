...
export type ReviewFile = DiffFile & {
	...
	// present); see isOversized in state/diff-files.ts. Omitted (not `false`) on ordinary files, so
	// the state
...
// server-side by /api/stage, /api/unstage, and readStagedSnapshot (state/reconcile.ts).
