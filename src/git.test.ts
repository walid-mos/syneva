...
void test('parseUnifiedDiff extracts the file and its hunk', () => {
...
void test('a hunk header without counts defaults each range to one line', () => {
	const diff = `diff --git a/f.txt b/f.txt
--- a/f.txt
+++ b/f.txt
@@ -1 +1 @@
...
void test('changeStableKeyFromBlock uses the first-line convention (side:line:dels:adds)', () => {
	const files = parseUnifiedDiff(DIFF)
...
void test('a pure addition keys on the deletions side count of zero', () => {
...
void test('a pure rename (zero hunks, distinct paths) is kept, from the rename headers', () => {
...
void test('a rename+edit keeps distinct paths AND its hunk', () => {
...
void test('a mode-only change (zero hunks, same path) stays dropped', () => {
...
void test('a same-path binary diff stays dropped', () => {
...
void test('a renamed binary (distinct paths + Binary line) is dropped, not read as text', () => {
