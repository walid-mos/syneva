...
```bash
node scripts/bench-dashboard.mjs            # benchmarks/dashboard.html (in-repo, commit it)
node scripts/bench-dashboard.mjs --check    # validate history + template, write nothing
node scripts/bench-dashboard.mjs --out ~/Desktop/syneva-perf.html   # personal copy
```
...
```bash
node scripts/bench-dashboard.mjs --record /tmp/run.json   # or "-" to read stdin
```
...
*duration* (the `/api/file-contents` transfer, measured by `scripts/browser-bench.mjs` from the
...
`node scripts/bench-fixtures.mjs` → `/tmp/syneva-bench/repos/{tiny,small,medium,large,bigfile,patho}`.
...
`scripts/browser-bench.mjs` is the older headful latency bench (scroll smoothness, memory). It is
