---
description: Plan work and discuss the live plan in Galley — the human comments on the plan and you answer and revise it in place
argument-hint: "<what to plan>"
---
Plan **$ARGUMENTS** and review the plan interactively through **Galley**: the plan lands in a browser desk where the human accepts/demands changes and asks questions; you revise the plan live. Iterate until the plan is approved — this command produces a plan, not an implementation.

Treat **`galley spec`** as the authoritative contract — run it once before your first Galley review this session (file mode, await/comment/reload loop, ReviewResult). `galley` is installed by the galley pi package (shim in `~/.pi/agent/bin`); if it is missing, tell the user and offer the upstream fallback `pnpm add -g galley-diff`.
...
2. **Write the plan** (English) to `<repo>/.pi/galley/plans/<slug>.md` where `<slug>` is from the request (kebab-case, ≤40 chars; if the file already exists, suffix `-$EPOCHSECONDS`). `mkdir -p` first. Sections: **Goal**, **Context** (what exists, relevant constraints), **Approach** (steps with rationale, in order), **Files touched** (per file: what changes and why), **Risks / open questions**, **Verification** (how each step proves done). If the repo has AGENTS.md/CLAUDE.md, honor it.
3. **Keep the plan invisible to git**: if in a git repo and the path is not already ignored (check `git check-ignore -q .pi/galley/plans/<slug>.md`), append one line `.pi/galley/plans/` to `.git/info/exclude` (never touch tracked `.gitignore` for this).
4. **Start the desk in the background** (file mode):
   ```bash
   rm -f /tmp/galley-<slug>.log; nohup galley file .pi/galley/plans/<slug>.md > /tmp/galley-<slug>.log 2>&1 &
