...
5. **End state.** `galley reload` after your edits against the re-checks if the reviewer requested tests; keep looping until the human approves everything or says done. When the review is over: detach with `galley_agent`, then run `galley stop` (idempotent) and summarize in chat: approvals left, changes applied, branch state.
